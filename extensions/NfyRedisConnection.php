...
/**
 * Based entirely on yii\redis\Connection from Yii 2.0 extensions, by Carsten Brandt <mail@cebe.cc>.
 *
 ...
 */
class NfyRedisConnection extends CApplicationComponent
{
	/**
	...
	public function open()
	{
		...
		Yii::trace('Opening redis DB connection: ' . $connection, 'nfy.'.__CLASS__);
		...
		if ($this->_socket) {
			...
			Yii::log("Failed to open redis DB connection ($connection): $errorNumber - $errorDescription", CLogger::LEVEL_ERROR, 'nfy.'.__CLASS__);
			...
			throw new CException($message, (int)$errorNumber);
		}
	}
	...
	public function close()
	{
		if ($this->_socket !== null) {
			$connection = $this->hostname . ':' . $this->port . ', database=' . $this->database;
			Yii::trace('Closing DB connection: ' . $connection, 'nfy.'.__CLASS__);
	...
	public function executeCommand($name, $params=array())
	{
		...
		Yii::trace("Executing Redis Command: {$name}", 'nfy.'.__CLASS__);
	...
	public function parseResponse($command, $ignoreTimeout=false)
	{
		...
		switch($type)
		{
			...
			case '*': // Multi-bulk replies
				...
				return $data;
			default:
				throw new CException('Received illegal data from redis: ' . $line . "\nRedis command was: " . $command);
		}
	}

	/**
	 * Backported from Yii 2 from yii\helpers\BaseInflector.
	 *
